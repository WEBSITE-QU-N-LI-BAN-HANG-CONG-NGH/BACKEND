...
public class OrderService implements IOrderService {
    ...
    public Map<String, Object> getOrderStatistics(LocalDate start, LocalDate end) {
        ...
        LocalDateTime endTime = end.atTime(23, 59, 59, 999999999);

        List<Order> filteredOrders = orderRepository.findByOrderDateBetween(startTime, endTime);

        long totalOrders = filteredOrders.size();
        double totalRevenue = filteredOrders.stream()
                .filter(order -> order.getOrderStatus() == OrderStatus.DELIVERED)
                .mapToDouble(order -> order.getTotalDiscountedPrice() != null ? order.getTotalDiscountedPrice() : 0.0)
                .sum();
        ...
        Map<LocalDate, Double> revenueByDate = filteredOrders.stream()
                ...
                .collect(Collectors.groupingBy(
                        order -> order.getOrderDate().toLocalDate(),
                        Collectors.summingDouble(order -> order.getTotalDiscountedPrice() != null ? order.getTotalDiscountedPrice() : 0.0)
                ));
        ...
        log.info("Order statistics calculated for period: {} to {}. Total orders: {}, Total revenue: {}", start, end, totalOrders, totalRevenue);
        return result;
    }
